...
export default function Sidebar({ currentPage, setCurrentPage, onLogout }) {
  ...
  return (
    <div className={`${isCollapsed ? 'w-20' : 'w-64'} h-screen sticky top-0 bg-[var(--bg-card)] border-r border-[var(--border-primary)] flex flex-col justify-between transition-all duration-300 shrink-0 font-sans select-none`}>
      
      {/* TOP SECTION */}
      <div className="flex flex-col h-full overflow-hidden">
        <div className={`flex items-center gap-3 px-6 mt-6 mb-8 ${isCollapsed ? 'justify-center' : 'justify-start'}`}>
          <div className="h-7 w-7 bg-purple-600 rounded-lg flex items-center justify-center shrink-0">
            <span className="text-white font-black text-sm">Ω</span>
          </div>
          {!isCollapsed && (
            <span className="text-lg font-bold tracking-tight text-[var(--text-primary)] whitespace-nowrap">
              Exam<span className="text-purple-500">OS</span>
            </span>
          )}
        </div>
        
        {/* NAVIGATION LINKS - SCROLLABLE CONTAINER WITH CUSTOM SCROLLBAR */}
          ...
          {Object.keys(iconMatrix).map((item) => {
            ...
            return (
              ...
              >
                ...
                <div className={`transition-transform duration-150 group-hover:scale-105 shrink-0 ${isActive ? 'text-purple-500' : 'text-[var(--text-secondary)] group-hover:text-[var(--text-primary)]'}`}>
                  {iconMatrix[item]}
                </div>

                {!isCollapsed && <span className="whitespace-nowrap">{item}</span>}
              </div>
            );
          })}
        </nav>
        ...
        {/* BOTTOM CONTROLS */}
        <div className="border-t border-[var(--border-primary)] p-4 space-y-2 shrink-0">
          {/* TOGGLE BUTTON */}
          <button
            onClick={() => setIsCollapsed(!isCollapsed)}
            className="w-full flex items-center gap-3 text-[var(--text-secondary)] hover:text-[var(--text-primary)] hover:bg-[var(--bg-input)] px-4 py-3 rounded-xl transition-all duration-150"
          >
          ...
          <button
            onClick={onLogout}
            className="w-full flex items-center gap-3 text-xs font-bold uppercase tracking-wider text-[var(--text-secondary)] hover:text-red-500 px-4 py-3 rounded-xl hover:bg-red-500/5 transition-all duration-150"
          >
